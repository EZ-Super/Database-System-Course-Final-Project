...
| bank_account     | STRING(255)  | N    | N        | 無                                       | 字串（建議格式驗證）          | 收款帳戶資訊                 |
...
| points_earned    | INT          | N    | N        | 無                     | 建議限制為整數，可為負值（扣點） | 當次增加或扣除的點數           |
...
| discount_amount | DECIMAL(10,2)  | N    | Y        | 無               | 正數，建議 `>= 0.00`                            | 折扣金額                       |
| expiry_date     | DATE           | N    | N        | 無               | 合法日期格式                                     | 優惠券到期日                   |
| usage_limit     | INT            | N    | N        | 無               | 整數，建議 `>= 0`                               | 可使用次數上限（NULL = 無限制）|
...
| 欄位名稱          | 建議限制方式                                                  | 理由說明                         |
|-------------------|---------------------------------------------------------------|----------------------------------|
| `coupon_code`     | 僅允許英數與基本符號（建議正規表示式）                        | 防止 XSS 與代碼格式錯誤          |
| `discount_amount` | 限制為大於等於 0 的浮點數                                     | 避免負數造成加價邏輯錯誤         |
| `expiry_date`     | 限制為合法日期格式，建議不得為過去時間（應用層邏輯）          | 保證活動邏輯正確                 |
...
| 欄位名稱            | 建議限制方式                                                   | 理由說明                         |
|---------------------|----------------------------------------------------------------|----------------------------------|
| `notification_type` | 僅允許英文、小寫字串，建議使用 ENUM 或白名單                  | 避免錯誤值與資料不一致           |
| `subject`           | 禁止 `<`, `>`, `"`, `'`，可使用正規表示式限制內容格式           | 避免 XSS 與顯示錯誤              |
| `message`           | 建議 HTML escape 或白名單過濾                                  | 避免 `<script>` 等惡意注入       |
| `related_entity`    | 建議使用 ENUM 或固定白名單值                                   | 避免模組名稱錯誤或注入           |
...
| `capacity`       | INT         | N    | Y        | 無               | 建議限制 ≥ 0                    | 倉儲容量                     |
...
| `location`        | 建議禁止 HTML 標籤，必要時進行 escape 處理                      | 避免位置欄位出現惡意內容或亂碼    |
...
| `stock_quantity` | INT         | N    | N        | 0                | 整數，建議限制為 `>= 0`               | 當前庫存數量                      |
| `reorder_level`  | INT         | N    | N        | 10               | 整數，建議限制為 `>= 0`               | 庫存補貨臨界值                    |
...
| 欄位名稱        | 建議限制方式                                             | 理由說明                         |
|-----------------|----------------------------------------------------------|----------------------------------|
| `supplier_name` | 禁止 `<`, `>`, `"`, `'` 等特殊符號，建議限長度與格式      | 避免 XSS 與顯示錯誤               |
...
| `quantity`      | INT       | N    | Y        | 無         | 建議限制為整數且 ≥ 0                         | 入庫數量                     |
...
| `arrival_date` | 必須為合法日期格式，建議不可早於系統日期（邏輯控制）     | 避免異常過期或未來日期錯誤       |
...
| `quantity`      | INT       | N    | Y        | 無           | 建議限制為整數且 ≥ 0                           | 出貨數量                     |
...
| `total_amount`  | DECIMAL(10,2) | N    | Y        | 無               | 建議限制為 ≥ 0.00                                          | 訂單總金額               |
...
| `quantity`      | INT           | N    | Y        | 無               | 建議限制為整數且 ≥ 1                         | 訂購數量                 |
| `price`         | DECIMAL(10,2) | N    | Y        | 無               | 建議限制為浮點數且 ≥ 0                        | 單項商品價格             |
...
| `shipping_cost`     | DECIMAL(10,2) | N    | N        | 0.00       | 金額格式，建議限制為 ≥ 0.00                        | 運費                         |
...
| `carrier`          | 禁止特殊符號、建議限長度                               | 避免 XSS 或資料庫混亂            |
...
| `product_id`         | INT       | N    | N        | 無                  | 外鍵參照 `Products(product_id)`                                | 商品 ID                      |
| `quantity`           | INT       | N    | Y        | 無                  | 建議限制為整數且 ≥ 1                                          | 調撥數量                     |
...
| `date`                 | DATE         | N    | Y        | 無             | 合法日期格式                            | 統計日期                        |
| `carrier`              | STRING(50)   | N    | Y        | 無             | 最長 50 字，建議限制字元類型            | 承運商名稱                      |
...
| `average_rating`   | DECIMAL(3,2)  | N    | Y        | 無             | 數值範圍建議為 1.00 ~ 5.00                | 平均評分（星數）                |
...
| `average_rating`   | 限制為浮點數，範圍建議為 1.00 至 5.00     | 確保評分合理                 |
...
| `amount`         | DECIMAL(10,2) | N    | Y        | 無                  | 金額格式，建議限制 ≥ 0                                              | 付款金額                     |
...
| `order_id`      | INT           | N    | N        | 無                  | 外鍵參照 `Orders(order_id)`                                       | 所屬訂單 ID                  |
| `product_id`    | INT           | N    | N        | 無                  | 外鍵參照 `Products(product_id)`                                   | 所屬商品 ID                  |
| `reason`        | STRING        | N    | N        | 無                  | 建議長度限制，並防止特殊符號                                     | 退貨/退款原因文字描述        |
| `status`        | ENUM          | N    | N        | `'requested'`       | 僅允許 `requested`, `approved`, `rejected`, `processed`           | 處理狀態                     |
| `refund_amount` | DECIMAL(10,2) | N    | N        | 無                  | 建議限制為浮點數且 ≥ 0                                           | 退款金額（可為部分退款）     |
| `created_at`    | TIMESTAMP     | N    | N        | CURRENT_TIMESTAMP   | 系統時間自動填入                                                | 建立時間                     |

| 欄位名稱        | 建議限制方式                                                | 理由說明                     |
|-----------------|-------------------------------------------------------------|------------------------------|
| `reason`        | 禁止 `<`, `>`, `"`, `'` 等特殊符號，建議轉義或濾除         | 避免 XSS 注入與亂碼           |
