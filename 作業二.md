...
VALUES 
(1, 10, 'warehouse', '新倉庫建立通知', '新倉庫「台中物流中心」已建立，ID: W001', 'warehouses', 'W001');
```
當新增新倉庫並完成設置後，系統通知倉儲主管
```sql
SELECT notification_type, subject, message, sent_at
FROM notifications
...
|------------------|-------------|-----------|------|
| warehouse	| 新倉庫建立通知	| 新倉庫「台中物流中心」已建立，ID: W001	| 2023-11-01 09:00:00 |

### 庫存管理
通知
```sql
INSERT INTO notifications VALUES (2, 11, 'inventory', '低庫存警告', '倉庫「台中倉」中產品 #P100 的庫存低於安全水平（僅剩 5 件）', 'inventory', 'inv-100');
```
庫存數量低於 reorder_level 時通知倉儲人員補貨
```sql
SELECT notification_type, subject, message, sent_at
FROM notifications
...
| notification_type	| subject	| message	| sent_at |
|---------|----------|-------|-----------|
| inventory	| 低庫存警告	| 倉庫「台中倉」中產品 #P100 的庫存低於安全水平（僅剩 5 件）	| 2025-05-06 10:01:00 |
...
### 入庫通知
當倉庫接收到商品時，系統會發送入庫通知給倉庫管理員
```sql
INSERT INTO notifications (notification_id, user_id, notification_type, subject, message, related_entity, related_entity_id)
VALUES (2, 5, 'inbound', '商品入庫', '商品 #20001 已到達倉庫 #A1，數量 100', 'inbound_shipments', 20001);
```
查詢使用者所有未讀通知
```sql
SELECT notification_type, subject, message, sent_at
...
| notification_type |	subject	| message	| sent_at |
|-------------------|------------------|-------------|----------------|
| inbound	| 商品入庫	| 商品 #20001 已到達倉庫 #A1，數量 100	| 2023-11-21 09:00:00|

### 出庫通知
當商品從倉庫出庫並且準備發貨時，系統會發送通知給倉庫管理員
```sql
INSERT INTO notifications (notification_id, user_id, notification_type, subject, message, related_entity, related_entity_id)
VALUES (3, 5, 'outbound', '商品出庫', '訂單 #10001 的商品已從倉庫 #A1 出庫，準備發貨', 'outbound_shipments', 10001);
```
查詢使用者所有未讀通知
```sql
SELECT notification_type, subject, message, sent_at
...
| notification_type |	subject	| message |	sent_at |
|------------------|---------------|-------------|-----------|
| outbound	| 商品出庫	| 訂單 #10001 的商品已從倉庫 #A1 出庫，準備發貨	| 2023-11-21 10:30:00 |


### 訂單發貨通知
當訂單已經發貨並且進入物流配送階段時，系統發送發貨通知給顧客

```sql
INSERT INTO notifications (notification_id, user_id, notification_type, subject, message, related_entity, related_entity_id)
VALUES (4, 2, 'shipment', '訂單已發貨', '您的訂單 #10001 已發貨，運單號碼：1234567890', 'shipments', 10001);
```
查詢使用者所有未讀通知
```sql
SELECT notification_type, subject, message, sent_at
...
| notification_type	| subject	| message	| sent_at |
|---------------|------------|-----------|-------------|
| shipment	| 訂單已發貨	| 您的訂單 #10001 已發貨，運單號碼：1234567890	| 2023-11-22 15:00:00 |

### 退貨與退款通知
當顧客申請退貨並獲得退款時，系統會發送退貨處理結果通知
```sql
INSERT INTO notifications (notification_id, user_id, notification_type, subject, message, related_entity, related_entity_id)
VALUES 
(5, 2, 'return', '退貨與退款處理結果', '您的訂單 #10001 退貨申請已處理，退款金額 $50.00', 'returns_refunds', 10001);
```
查詢使用者所有未讀通知
```sql
SELECT notification_type, subject, message, sent_at
...
| notification_type	| subject	| message	| sent_at |
|------------|----------|---------|---------|
| return	| 退貨與退款處理結果 |	您的訂單 #10001 退貨申請已處理，退款金額 $50.00	| 2023-11-23 16:00:00 |
